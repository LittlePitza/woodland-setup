  ...
  return (
    <main className="min-h-screen px-4 py-6 sm:py-10 max-w-3xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <h1 className="font-display text-4xl text-ink">Partidas anteriores</h1>
        <p className="font-body text-ink-soft mt-2">
          {history.length === 0
            ? "Aún no hay partidas guardadas."
            : `${history.length} ${
                history.length === 1 ? "partida guardada" : "partidas guardadas"
              }.`}
        </p>
      </div>
      ...
      {history.length === 0 ? (
        <div className="text-center py-16 bg-paper-light/50 rounded-lg border border-ink/10">
          ...
          <p className="font-body text-ink-soft mb-6">
            El bosque aún está en silencio.
          </p>
          <Link
            href="/setup"
          ...
          <div className="mt-10 text-center">
            <button
              ...
              className={cn(
                ...
                confirmClear
                  ? "bg-rust text-paper-light"
                  : "text-ink-muted hover:text-rust"
              )}
            >
              {confirmClear ? "¿Borrar todo? Confirma" : "Borrar historial"}
            </button>
          </div>
        </>
      )}
...
function SetupHistoryCard({
  ...
  const mapName = setup.mapId
    ? MAPS.find((m) => m.id === setup.mapId)?.nameES
    : null;
    ...
    <div className="bg-paper-light/80 rounded-lg p-4 sm:p-5 border border-ink/15 shadow-card">
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        {setup.factions.map((af) => {
          ...
          return (
            ...
            >
              <span style={{ color: faction.color }} aria-hidden>
                {faction.symbol}
              </span>
              <span className="text-ink-soft">{faction.nameES}</span>
            </span>
          );
        })}
      </div>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-ink/10">
        <span className="font-ui text-xs text-ink-muted">
          ...
          <span
            className={cn(
              "font-display font-bold text-base ml-1",
              setup.isBalanced ? "text-moss" : "text-rust"
            )}
          >
            {setup.totalReach}
          </span>
        ...
        >
          {setup.isBalanced ? "✓ Balanceada" : "⚠ Bajo umbral"}
        </span>
      </div>
    </div>
  );
